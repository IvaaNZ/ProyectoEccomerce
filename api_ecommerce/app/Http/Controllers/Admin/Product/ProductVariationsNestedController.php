...
class ProductVariationsNestedController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->propertie_id){
            ...
            $is_valid_variation = ProductVariation::where("product_id",$request->product_id)->where("product_variation_id",$product_variation_id)
                                                        ->where("attribute_id",$request->attribute_id)
                                                        ->where("value_add",$request->value_add)
                                                        ->first();                                          
        }
        if($is_valid_variation){
            return response()->json(["message" => 403,"message_text" => 'La variación ya existe, intente otra combinación.']); 
        }
        ...
        $SUM_TOTAL_STOCK_NESTED = ProductVariation::where("product_id",$request->product_id)
                                                    ->where("product_variation_id",$product_variation_id)
                                                    ->sum('stock');
        $SUM_TOTAL_STOCK_NESTED = $SUM_TOTAL_STOCK_NESTED + $request->stock;

        if ($SUM_TOTAL_STOCK_NESTED > $TOTAL_STOCK_VARIATION_CENTRAL) {
            return response()->json([
                "message" => 403,
                "message_text" => "No es posible crear la Variación, porque el Stock supera a su Variación padre."]);
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($variations_exits > 0){
            $variations_attributes_exits = ProductVariation::where("product_id",$request->product_id)->where("product_variation_id",$product_variation_id)
                                            ->where("attribute_id",$request->attribute_id)
                                            ->count();
            if($variations_attributes_exits == 0){
                return response()->json(["message" => 403,"message_text" => "No es posible agregar un Atributo diferente del que se ha creado en la lista."]);
